...
class XmlDoc:
    ...
    def extractInfo(self):
        ...
        styleNodeList = [stlNode for stlNode in self.xmlDOM.getElementsByTagName(self.XML_TAG_STYLE) if (stlNode.nodeName == self.XML_TAG_STYLE)]
        self.STYLE_NODE = styleNodeList
        pageList = []
        for pageNode in pageNodeList:
            ...
            page["StyleList"] = self.extractStyle()
            self.PAGE_NODE = pageNode
...
class JsonDoc:
    ...
    def getTableAttrList(self):
        ...
        return tableAttrList, rowAttrList, cellAttrList, lineAttrList, pageAttrList
    ...
    def getTableText(self):
        ...
        if len(self.getRawJson()[0]["TableList"]) == 0:
            ...
            tAttrList = self.getTableAttrList()
            tableAttrList = tAttrList[0]
            rowAttrList = tAttrList[1]
            cellAttrList = tAttrList[2]
            lineAttrList = tAttrList[3]
            pageAttrList = tAttrList[4]
            ...
            rawDF = json_normalize(self.getRawJson(), ["TableList", "RowList", "CellList", "LineList", "WordList"], pageAttrList + tableAttrList + rowAttrList + cellAttrList + lineAttrList)
            ...
            colNames = ["PageID", "PageNo", "Lang", "XDPI", "YDPI", "PageStartX", "PageStartY", "PageWidth", "PageHeight", \
                        "PrintAreaStartX", "PrintAreaStartY", "PrintAreaWidth", "PrintAreaHeight", \
                        "TableID", "TableStartX", "TableStartY", "TableWidth", "TableHeight", "TableRows", "TableCols", "TableStyle", \
            ...
            return (rawDF.groupby(["PageNo", "TableID", "RowID", "CellColumnID"], sort=True, as_index=False)
    ...
    def enrichTableDataFrame(self):
        ...
        enrichedTableDataFrame = pd.merge(rawTableDataFrame, lineTextDF, on=["PageNo", "TableID", "RowID", "CellColumnID", "LineID"], how="left", suffixes=("_Raw", "_Line"))
        ...
        """
        enrichedTableDataFrame.sort_values(["PageNo", "TableID", "RowID", "CellColumnID", "LineID", "WordID"], ascending=[True, True, True, True, True, True])
        ...
        tempDF = pd.DataFrame()
        tempDF = (enrichedTableDataFrame.groupby(["PageNo", "TableID", "RowID", "CellColumnID", "LineID"], as_index=False)
                  .mean()
                  .rename(columns = {"WordDistance": "AvgWordDistance"}))
        tempDF = tempDF[["PageNo", "TableID", "RowID", "CellColumnID", "LineID", "AvgWordDistance"]]

        # Join back with the original dataframe
        enrichedTableDataFrame = pd.merge(enrichedTableDataFrame, tempDF, on=["PageNo", "TableID", "RowID", "CellColumnID", "LineID"], how="left")
    ...
    def getTableTextByLine(self):
        if len(self.getRawJson()[0]["TableList"]) == 0:
            ...
            return (rawDF.groupby(["PageNo", "TableID", "RowID", "CellColumnID", "LineID"], sort=True, as_index=False)
                    .agg({"WordValue": lambda x: " ".join(x).strip()}))
...
class ProcessKeyValue:
    ...
    def process_input(self):
        ...
        # print self.TableDataFrame.head(5)
    ...
    def DataFrame_analytic(self):
        ...
        Block_dataframe = self.enrichedDataFrame[
            ['PageNo','WordID', "WordValue","FontColor", "FontSize", "FontFamily", "FontName",
